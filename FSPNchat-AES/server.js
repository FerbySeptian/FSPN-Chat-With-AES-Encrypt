...
function processJsonRequest(jsonString) {
  ...
  if (chan.match(/^[a-f0-9]{64}$/i) === null) return "{}";

  switch (func) {
    case "get":
      var t = parseInt(json["t"]);
      if (t < 0) return;
      if (!(chan in chatLines)) return;
      var log = {};
      ...
      for (i = t + 1; i < lastLine + 1; i++)
        if (i in chatLines[chan])
          log["text"] +=
            chatLines[chan]["time" + i] + "," + chatLines[chan][i] + "\n";
      console.log("Get message: ", JSON.stringify(log));
    ...
    case "post":
      ...
      var lastLine = 1;
      if (chan in chatLines) lastLine = chatLines[chan][0] + 1;
      else chatLines[chan] = {};
      chatLines[chan][0] = lastLine;
      chatLines[chan][lastLine] = line;
      chatLines[chan]["time" + lastLine] = new Date().getTime() / 1000;
      var log = {};
      log["t"] = lastLine - 1;
      ...
      console.log("Post message: ", JSON.stringify(log));
  }
  return JSON.stringify(log);
}
  ...
  .createServer(function (request, response) {
    ...
    fs.readFile(filename, "binary", function (err, file) {
      ...
      if (uri == "/xkcd_pw-min.js") {
        ...
        var hash = crypto
          .createHash("sha1")
          .update(new Date().getTime() + "slkfjsdleiu")
          .digest("hex");
        file += 'var xkcd_pw_gen_server_hash="' + hash + '"';
        console.log("Encrypted hash for xkcd_pw-min.js: " + hash);
      }
      if (uri == "/emo.js") {
